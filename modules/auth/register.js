...
const register = (req, res) => {
  ...
  userModel.findOne({$or:[{"username":req.body.username}, {"email":req.body.email}, {"phone":req.body.phone}]}, (err, user) => {
    if(user) {
      success = false;
    ...
    bcrypt.hash(req.body.passEnter, req.app.get('salt'), (err, passHash) => {
      ...
      if (success) {
        ...
        res.json({ "success": success, "message": 'Registration failed.', "errors": errors });
      }
    });
  });
}
